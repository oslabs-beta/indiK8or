  ...
  */
    const vulnerabilities =
      (typeof scannedImages === "string"
        ? []
        : scannedImages?.matches?.map((el) => el.vulnerability)) || [];
        ...
        <TableContainer id="modal-modal-description">
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              ...
              <TableRow>
                <TableCell
                  className="modal-table-row-head"
                  sx={{ fontWeight: "bold" }}
                >
                  ID
                </TableCell>
            ...
            <TableBody>
              {vulnerabilities.length === 0 ? (
                ...
                vulnerabilities.map(
                  (vulnerability: VulnerabilityProps, index: number) => (
                    <TableRow key={index}>
                      <TableCell id="v-id">
                        <a href={vulnerability.dataSource} target="_blank">
                          {vulnerability.id}
                        </a>
                      </TableCell>
                      <TableCell id="v-desc">
                      ...
                      <TableCell
                        id="v-sev"
                        className={getSeverityClassName(vulnerability.severity)}
                      >
